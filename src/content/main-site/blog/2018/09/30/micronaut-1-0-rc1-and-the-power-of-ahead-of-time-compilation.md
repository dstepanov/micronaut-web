---
slug: 2018/09/30/micronaut-1-0-rc1-and-the-power-of-ahead-of-time-compilation
...
date: '2018-09-30T12:48:28'
modified: '2021-02-19T18:00:23'
sourceUrl: https://micronaut.io/2018/09/30/micronaut-1-0-rc1-and-the-power-of-ahead-of-time-compilation/
wordpressId: 2922
...
tags:
  ...
  - openapi
  - release
href: /2018/09/30/micronaut-1-0-rc1-and-the-power-of-ahead-of-time-compilation/
---

Today, it is with great pleasure that the team at [Object Computing, Inc.](https://objectcomputing.com/) announces the release of Micronaut 1.0 RC1.

Over a year in the making, the Micronaut framework represents a significant step forward in microservice frameworks for the JVM by supporting all of the features Java developers know and love, such as dependency injection (DI) and aspect-oriented programming (AOP), without making compromises on startup time, performance, and memory consumption.
...
- Most frameworks are not aware of the diverse language ecosystem on the JVM. Languages such as Groovy and Kotlin produce more methods at the byte code level than Java does to support the additional language features they provide. This difference means that reflection caches are often larger for Groovy- and Kotlin-based classes, since the byte code contains more methods, fields, and so on.

One of the key goals we set out to achieve when developing the Micronaut framework was to enhance Java’s efficiency and make it viable and possible to run applications that contain no compromises in terms of modern framework features but occupy 10s of megabytes of memory instead of 100s of megabytes.

Although the Micronaut framework has many features that are undeniably beneficial to the creation of microservices, the Micronaut framework is, in fact, a general purpose framework that can be used to build a range of applications, from CLI applications, to functions that run in the cloud, to scheduled jobs that process data from Kafka.

The Micronaut framework features a custom-built [DI](https://docs.micronaut.io/1.0.x/guide/index.html#ioc) and [AOP model](https://docs.micronaut.io/1.0.x/guide/index.html#aop) that *does not* use reflection.
...
But not with the Micronaut framework:
...
This feature is powered by the Micronaut framework’s extensive support for compile-time visitors that validate you are using the framework correctly and provide early feedback to obvious framework-related problems in your code.

In addition, this checking mechanism works whether you are a Java, Kotlin, or Groovy developer. The Micronaut framework’s abstraction across language ASTs ensures a consistent experience regardless of which language you choose.

## Feature 4: Mapping Annotations at Compile Time

The approach we took in the design of the Micronaut framework necessitated the creation of a new set of Java annotations enhanced to trigger AOT compilation.

Unlike traditional Java frameworks, such as Java EE (now Jakarta EE) and Spring, that perform analysis of framework-level annotations at runtime, the Micronaut framework performs this same analysis but at compilation time, allowing startup performance to be faster (particularly as your application grows) and memory consumption to be significantly lower.

There are, however, a number of excellent specifications and standards in the Java space that developers are already comfortable with; it made sense to support these in the Micronaut framework. Thanks to the Micronaut framework’s extensive annotation metadata, you can map the values of any annotation to the values of any other annotations at compile time using an AnnotationMapper.

What this means in practice is the Micronaut framework can support annotations such as `javax.annotation.security.RolesAllowed` for security or `javax.persistence.PersistenceContext` for injecting a JPA `EntityManager`.
...
Whether you are building a client or a server, a scheduled job or a stream processing application, the Micronaut framework has a compelling set of features that you can introduce without paying the heavy cost associated with most current Java frameworks.
