...
A guide's snippets resolve against the guide's own directory and, when it exists, the sample project the guides build writes to `build/code/<slug>/<slug>-<build tool>-<language>` in the guides checkout. That project is the `sourcedir` the upstream Asciidoctor build uses, and it is the only place to read the Micronaut Starter files a guide never checks in, such as the application class, the generated configuration, and the build scripts. `npm run render:guides` works without it; only those Starter snippets stay unresolved. Publishing generates it with `./gradlew generateCodeZip`, which also writes the per-variant archives each guide links as its download. A guide is never offered as a Maven and Kotlin variant, because the guides build refuses to generate that combination.
...
The web workflow uses the repository's GitHub Pages Actions permissions (`pages:write` and `id-token:write`) and does not need a branch-publish token. Docs and guides push to a different repository, so both require `GH_TOKEN` with `contents:write` access to their publication target.
...
The docs workflow resolves the Platform ref explicitly as a branch first, then as an exact tag, then as a `v`-prefixed tag such as `v4.10.14`, and finally as a raw Git ref or SHA. The ref comes from `client_payload.sha` on a platform release and otherwise from `docs_version`, so publishing `docs_version=4.10.14` resolves the Platform tag `v4.10.14`.
...
- `DEFAULT_GITHUB_PAGES_ORIGIN`: computed by GitHub Actions from the workflow repository owner, for example `https://${GITHUB_REPOSITORY_OWNER}.github.io`. The current docs or guides surface URL is derived separately from the publication repository.
...
Guides are currently published as a latest-only surface. The guides workflow accepts the guides repository/ref to build from, checks out `micronaut-projects/micronaut-guides-v2` on `gh-pages`, builds `npm run build:guides`, replaces the published branch contents with the pruned `dist` plus a `.guides-revision` marker recording the built commit, and pushes `gh-pages`. It does not check out Micronaut Platform or render generated docs.
...
Use a React island only when the component owns meaningful client state, keyboard handling, focus management, or third-party interactive primitives. Current docs pages intentionally hydrate `SiteHeader`, `DocsSidebarRailControl`, and `DocsSidebarMobileSheet`. The guides catalog renders each card from `src/components/web/latest-guide-card.astro` and hydrates only `GuideVariantMenu`, and only for guides that offer more than one variant. Docs scroll-spy behavior lives in `src/components/web/docs-scroll-spy.astro` as a processed Astro script. Do not reintroduce a docs version switcher island; `src/components/web/docs-version-selector.astro` renders fallback options statically and uses a processed Astro script to fetch `/versions.json`.
